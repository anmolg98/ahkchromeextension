...
function keydownHandler(event) {
    ...
    if ((!(pressedKeyTrack.has(pressKey))) && currentPressed.length == pressedKeysCount) {
...
function calculatechars(shortcut) {
    ...
    for (var index = 0; index < shortcut.length; index++) {
        if ((shortcut[index] >= 65 && shortcut[index] <= 90) || (shortcut[index] >= 48 && shortcut[index] <= 57) || (shortcut[index] >= 219 && shortcut[index] <= 222) || (shortcut[index] >= 186 && shortcut[index] <= 192)) {
            ans++;
        }
